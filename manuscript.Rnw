...
<<echo = FALSE, fig.cap = '\\label{Fig:massloss} Mass across temperature for \\textit{Juncus amabilis}.', out.width = '.6\\linewidth', fig.align = 'center'>>=
...
<<echo = FALSE, fig.cap = '\\label{Fig:fs_simulation} Parametric study of the Fraser-Suzuki function for deconvolution of derivative thermogravimetric biomass curves: Effect of modifying (a) height; (b) skew; (c) position; and (d) width.', out.width = '\\linewidth', fig.asp = 1>>=
...
<<echo = FALSE, fig.cap = '\\label{Fig:stt_examples} Mass loss and component estimation using default settings for test samples.', out.width = '.9\\linewidth', fig.align = 'center', fig.show = 'hold'>>=
knitr::include_graphics('figs/default_tests.png')
@

Default \verb|mixchar| settings did not, however, perfectly identify the fourth peak in the deconvolution of macroalgae species \textit{Ecklonia radiata} blades \citetext{Fig.~\ref{Fig:Er}a; \citealp{trevathan-tackett2015}}. In this case, we can use the option to specify our own starting values, with the \verb|start_vec|, \verb|lower_vec|, and \verb|upper_vec| arguments, in order to better guide the model (Fig.~\ref{Fig:Er}b).\\
...
<<echo = FALSE, fig.cap = '\\label{Fig:Er} Component estimation for \\textit{Ecklonia radiata} blades with default (a) and specified starting values (b).', out.width = '.9\\linewidth', fig.align = 'center', fig.show = 'hold'>>=
...
<<echo = FALSE, fig.cap = '\\label{Fig:decon} Deconvolution of \\textit{Juncus amabilis} example dataset. Mass loss data overlaid with output of deconvolution. Rate of mass loss scaled by initial mass of sample.', asp = 1, out.width = '.7\\linewidth', fig.align = 'center'>>=
